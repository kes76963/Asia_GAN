...
def getRecommendation(cosine_sim):      # 코사인 유사도롤 활용하여 유사한 영화 추천하는 함수
    ...
    hospital_idx = [i[0] for i in simScore]     # 인덱스(i[0]) 뽑아서 리스트 생성
    recMovieList = df_hospital_review_one_sentence.iloc[hospital_idx]        # df에서 해당 병원 리스트 추출
    return recMovieList


#병원명 검색
hospital_idx = df_hospital_review_one_sentence[df_hospital_review_one_sentence['names']=='김이비인후과의원'].index[0]      # 영화 제목으로 인덱스 값 찾기
# movie_idx = 127
